...
Let's dive a bit in the interpretation with an example.
...
The software returns a probability for each type it has: the higher, the more confident it is of the assignment. These are our fuzzy value:
...
There must be some cell misclassification: we have `r nElements(sc_classification)` cells in this example but there are `r sum(sample_cells$size)` cells that the maximum probability predicted for these types is a single cell type. 
